...
| `tasks/<stage-id>/<task-id>/submission-debug.json` | Agent submission lifecycle checkpoints and command telemetry for the task. |
...
| `tasks/<stage-id>/<task-id>/tool-use-evidence.json` | Required when a task selects advisory `mcp.*` tools; accounts for selected, used, attempted, unavailable, and fallback tool evidence. |
